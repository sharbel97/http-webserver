...
request_type parse_request(recv_buffer *ps) {
  ...
  if (strstr(ps->requestbuf, "\r\n\r\n")) {
    if (strstr(ps->requestbuf, "GET /index.html HTTP/1.1")==ps->requestbuf|| strstr(ps->requestbuf, "GET / HTTP/1.1")==ps->requestbuf) {
      // if (strstr(ps->requestbuf, "GET /index.html HTTP/1.1")) {
      //   if (ps->requestbuf[0] != 'G' && ps->requestbuf[1] != 'E' && strlen(ps->requestbuf) > 24) return bad;
      // }
      // if (strstr(ps->requestbuf, "GET / HTTP/1.1")) {
      //   if (ps->requestbuf[0] != 'G' && ps->requestbuf[1] != 'E' && strlen(ps->requestbuf) > 14) return bad;
      // }
      return root;
    }
...
void check_clients(pool *p) {
  ...
  for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
    ...
    if ((connfd > 0) && (FD_ISSET(connfd, &p->ready_set))) {
      ...
      size_t x = read(connfd, rb->requestbuf+rb->first_empty_byte, 4096-rb->first_empty_byte);
      rb->first_empty_byte+=x;

      // client ended conversation
      if (x == 0) {
      ...
      switch (parse_request(rb)) {
        ...
        case speak:
          ...
          // loop over  all listening clients, and written the message to them
          
          extract_message(rb->requestbuf, buf);

          for (j = 0; (j <= p->maxi); j++) {
            if (p->receiving_events[j] == 1) {
              writen(p->clientfd[j], buf, strlen(buf));
            }
          }
          writen(connfd, OK, strlen(OK));
          close_and_remove(p, i);
